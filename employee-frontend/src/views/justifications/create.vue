...
<template>
  <div class="p-8 bg-gray-100 min-h-screen">
    <h1 class="text-3xl font-bold mb-6">Justifications</h1>

    <div class="bg-white rounded-2xl shadow p-6 mb-8">
      <div class="grid md:grid-cols-2 gap-4">
        <input v-model="form.absence_id" placeholder="Absence ID" class="border p-3 rounded-xl" />

        <input v-model="form.justified_by" placeholder="User ID" class="border p-3 rounded-xl" />

        <select v-model="form.type" class="border p-3 rounded-xl">
        ...
        <select v-model="form.status" class="border p-3 rounded-xl">
    ...
    <div class="bg-white rounded-2xl shadow overflow-hidden">
      <table class="w-full">
        ...
        <tbody>
          <tr v-for="j in justifications" :key="j.id" class="border-b">
            <td class="p-4">
              {{ j.id }}
            </td>
